...
export async function GET() {
  try {
    ...
    const response = await ai.models.generateContent({
      ...
      contents: "Generate exactly 20 multiple-choice questions about personal finance, investing, budgeting, and credit. Mix difficulties evenly: 7 Easy, 7 Medium, 6 Hard. Each question must have exactly 4 answer options. Return raw JSON only, adhering to exactly this schema: { questions: [{ id: number, difficulty: string, question: string, options: string[], correctIndex: number, explanation: string }] }. No markdown blocks, no extra text.",
      config: {
